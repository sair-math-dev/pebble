...
同一批已检查声明与实际直接依赖导出 [定理依赖 DAG](theorem-index.md)，供 agent 读取节点并沿依赖/反向使用关系探索。名称、文本和符号索引帮助定位入口。导出关联完整作用域、环境、精确前提端点和证据绑定；内部 arena 节点 ID 不作为跨进程身份。图与入口特征是非权威投影，不加入内核接受路径。
