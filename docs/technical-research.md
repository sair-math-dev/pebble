...
当前范围修订：写作编辑器与实时共同编辑不在本期范围。本期核心是 Cargo 式本地包工作流、注册/验证发布，以及 GitHub 风格的包浏览。下面完整 Git 托管、PR、社交和训练相关内容作为后续研究保留，不是首发依赖。
...
写作编辑器、CRDT、WebSocket 共同编辑、离线草稿同步及 Git/实时草稿合并全部移出当前技术选型，不预建这些服务或数据表。
