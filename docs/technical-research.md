...
本方案细化 [产品设计](product-architecture.md)。外部组件能力来自所链接的官方资料；尚未落地的选型、接口和资源目标仍是建议。2026-09-09 已验收迁移前的 HTTP 快照注册/发布、隔离 Slate 检查与 Rust 客户端依赖复用，实测范围见 [实施记录](implementation-status.md)。用户随后明确完整 Git 为版本基础；当前运行代码、数据库 schema 和部署配置尚未实现 Git 托管及 commit→snapshot 绑定。Git 部分目前只有架构修订，尚未实现或部署公网；后续邀请制增量见第 7 节。早期 [接入实验](../experiments/README.md) 保留作底层回归；当前可调用协议以 [实施契约](registry-protocol.md) 为准。

产品边界：写作编辑器与实时共同编辑归 Clay；Pebble 提前设计接入接口，自身不提供这些功能。本期核心是完整 Git 研究仓库、Cargo 式本地包工作流和注册/验证发布。clone、fetch、push、历史、分支和标签是基础能力；复杂 PR、社交和训练相关内容仍按具体任务推进。
...
| 源码 | 完整 Git 仓库 | 以稳定 `repo_id` 和精确 commit 保存版本事实；包快照从该 commit 及包路径导出，具体 Git 后端仍待选型和实现 |
| 文件 | S3 API 对象存储与 Git 持久存储 | 对象存储保存派生交付内容、证据及固定附件；Git 保存源码、refs 和完整历史，两者均须纳入恢复验证 |
| 定理发现 | 定理依赖 DAG、正反向邻接 API、简单入口索引 | agent 自主选择探索路径；先用 PostgreSQL 邻接表，语义入口和图数据库按实测需要决定 |
...
上图描述修订后的目标架构，Git 部分尚未落地。逻辑模块不等于独立微服务；Web、API、任务协调器可来自同一 TypeScript workspace，验证独立运行以隔离用户内容。

早期建议仓库布局如下；当前实际采用 `src/` 内的 API、数据库和 worker 模块、`migrations/`、`deployment/` 与 `tests/`。尚无 Web 应用，暂不为下面每个目录新建空壳：
...
Pebble 必须支持真实 clone/fetch/push、分支、标签和完整历史。源码页与发布候选来自 Git；发布及依赖固定稳定 `repo_id`、精确 commit 和必要的包路径，不能跟随 `main` 或标签漂移。现有上传快照接口须迁移到 Git 提交或固定 commit 导出路径，不能继续维护独立于 Git 的源码事实。以下后端比较仍是提案，不代表已有 Git 服务。
...
| 原生 Git 服务 + Pebble 数据模型 | Git 传输、对象、diff、merge、ref 事务 | 托管运维、分支保护、权限、限额、备份均由我们负责 | 核心 Git 后端候选，与通用托管组件比较后决定 |
...
仓库以不可变 `repo_id` 标识，并关联项目；采用 bare repository 时以该 ID 定位存储目录；用户输入的 owner/slug 只查映射，不能拼进文件路径或 shell。对象读取使用 `git cat-file --batch`，树枚举用 `git ls-tree -z`，避免每个文件启动一个进程；所有 Git 子进程用固定 argv、受控环境和超时。

HTTP gateway 校验短期/可撤销 token，生成绑定 project、操作、权限修订和请求 ID 的内部授权。Git backend 不公开监听，只接收受控 gateway 的请求；仓库级读权限作用于整个对象库。clone/fetch、ref 广告和历史读取也检查当前权限；push 在平台控制的 ref 更新阶段复查权限和保护规则，并与撤权协调，不能只依赖连接建立时的授权。禁止用户安装服务器 hook，用户仓库内的文件不成为 hook。导入 URL 的网络访问与构建隔离，限制协议、重定向和内网地址。
...
Git 与 SQL 无共同事务：先建立含 operation ID 的 SQL 操作记录，Git 侧更新目标 ref 并记录操作完成标识，再由幂等 reconciler 完成 SQL 投影。系统不得在 ref 更新成功、SQL 写入失败后再次无条件提交。发布固定候选 commit 及其导出包，保留已发布 commit 的可恢复绑定；分支前进、标签移动或删除不会改写旧 release 和引用。同一仓库在一次验证中采用同一 commit 的完整源码，不按各定理最后修改的 commit 混装。Git 内多个 ref 可事务更新，但这不解决 Git/SQL 的跨存储原子性。

### 仓库删除与发布归档

普通删仓关闭开发仓库的读写入口，并保留不可复用的 `repo_id` 及删除状态；未发布任务停止取得正式发布资格。已发布成果由平台管理的归档保留，不能通过删除仓库、账户或所在存储目录级联清除。仅在原 bare repository 内加受保护 ref 不足以满足这个要求。保留义务在正式发布时建立，不以当前站内反向依赖计数大于零为条件，站外引用和并发发布不能由该计数覆盖。

正式发布前，先在独立于开发仓库生命周期的存储中持久保存所选 commit、所需 Git 对象闭包、派生包、数据、工具链及证据关联，并核查完整锁定依赖均有相应保留绑定。归档使用原 `repo_id`、commit OID 与内容摘要，不生成可另行修改的源码版本。Git 对象可共享物理存储，但归档不能只借用即将删除的仓库对象目录；垃圾回收必须尊重发布保留关系。可用自包含 Git bundle 或独立归档仓库保存相同对象；含外部 prerequisites 的增量 bundle 不能单独充当可恢复档案。[Git bundle](https://git-scm.com/docs/git-bundle)。具体后端尚未选定。

归档接纳前按稳定账户/组织及平台总预算原子预留字节、对象/版本数和历史成本，再准备对象；暂存资源也有独立上限。预留绑定幂等操作 ID，发布事务将其转为长期占用。失败或过期准备须终止在途写入并清理实际对象后再释放；SQL 回滚本身不表示存储已经回收。普通删仓、改名或重建不返还已发布占用；保留承诺须由有限免费额度、机构预算或获批容量承担，不能无条件无限免费接纳。

归档准备期间保护对象免于并发回收。发布与删除在同一仓库状态/锁协议下确定顺序：删除先完成则拒绝发布；发布先完成则删仓保留已建立的档案。恢复演练必须验证无需原仓库即可取得相同 commit 与依赖内容，不能将跨 SQL/Git 的步骤假定为单一事务。资源预留与长期占用记账尚未实现。

仓库 ACL 与发布的当前分发授权分别表达：正式公开发布要明确确认选定内容的归档与公开分发；之后普通删仓不自动撤销这项分发。私有档案仍逐次检查当前授权，不能因归档或已知 hash 变为公开。来自私有仓库的公开发布只能读取被授权的交付内容，不能直接暴露可能包含私有祖先历史的 Git bundle。必要的内容下架/清除独立执行并阻止后续服务读取；保留允许的最小引用墓碑，将内容不可获取与数学验证撤销分开。

包客户端以稳定发布/包身份解析归档位置；旧 repo 删除后，锁文件仍绑定原 `repo_id + commit`，不通过原 owner/slug 寻找替代。同名新仓库必须分配新 ID，精确引用使用含稳定身份的落点，不能让名称复用接管旧引用。以上均为待实现约定；当前运行代码尚无 Git 仓库删除、独立 Git 发布归档或仓库/发布分发权限拆分。
...
论文由外部工具产出，以 Git 文件或 commit 中固定的附件引用进入项目；上传入口也须形成 Git 修订。首版建议接受 PDF 和 Markdown：PDF 提供阅读/下载，Markdown 安全渲染公式和链接；不要求平台支持可逆编辑格式或多人草稿。

上传附件先进入 staging，经类型、长度和摘要检查后形成不可变对象。论文记录绑定仓库、commit、路径及实际文件/对象摘要；正式发布绑定该记录。上传新版创建新记录，不替换旧发布引用的文件。Markdown 所需图片等资源也固定内容摘要，不能依赖会变动的远程文件完成发布阅读。
...
| 固定 Git 成果修订提交 | 将论文、代码、数据引用作为一个仓库 commit 交给 Pebble | 返回仓库、commit 及派生内容摘要；不接收光标、击键或 CRDT updates |
...
成果提交统一进入 Git 修订与 release candidate 路径，补充 project revision 查询/提交与权限查询。发布候选绑定仓库、精确 commit、包路径和导出快照；现有独立快照上传接口须随 Git 接入迁移。接口使用公开版本化 schema，论文资源格式可声明媒体类型，不绑定某一种编辑器内部文档树。

Clay 的草稿、实时会话和研究记录是不同对象。若以后接入研究记录贡献，走有独立公开/训练选择的记录 API，不能因提交论文快照而顺带上传对话或实时编辑历史。平台提供研究讨论不意味着承担 Clay 的共同写作。

首个 API 集成验收可由普通客户端完成：读取精确 Git commit → 在外部修改 → commit/push → 从固定 commit 获取检查结果 → 授权发布；同时检查过期基线、撤权和重复请求。这个验收验证 Pebble 已为 Clay 做好接入准备，不需要实现任何编辑器。

## 4. 数据模型与事务约束

以下是完整方向的逻辑表，不是已经执行的 migration。Git 接入需要仓库身份、Git 操作与 commit→snapshot 绑定，连接既有包归属/权限、候选、检查、发布、任务与下载记录；PR、社交、竞赛和训练表随对应功能另行引入。大对象以摘要引用存储，授权判断依靠关系表。
...
| `repositories`, `git_operations` | 稳定 repo ID、project 关联、Git 操作 ID、expected/new OID、状态 |
| `source_bindings` | 仓库、精确 commit、包路径与派生 snapshot digest；发布和依赖保留这一固定绑定 |
...
发布事务锁定候选和涉及的 package 行，复查 Git commit 与派生内容绑定、当前发布者权限、检查/审核绑定、依赖撤销状态及当前政策；插入所有 package_versions、release 与 outbox 后一次提交。任何一个包版本冲突则整个发布不生效。发布者撤权、包转移和政策更新也使用相同的版本/锁协议，避免检查与提交之间更换授权。
...
定理发现以精确版本依赖 DAG 为核心，agent 通过入口查找、批量节点读取和正反向邻接自主探索。数据导出、图构建、权限与增量更新见 [定理 DAG 与 agent 探索](theorem-index.md)。它复用 result revisions，不建立第二套 theorem 身份或证明格式。
...
评测集从真实研究任务整理：精确符号、同义描述、中文/英文、带假设限制、找前置引理及多定理组合。在相同目标/入口和总预算下比较平面列表与 agent 图探索，记录真实使用成功率及探索代价。向量只是可选入口能力，继承项目读取限制。
...
初期账户采用邀请制。已实现运维签发随机、单次、限时、可撤销邀请，持久化保存摘要及签发/使用/撤销状态；兑换原子建立账户，同凭据重试返回相同账户。用户不获得签发权，不开放自主注册或无限转邀。邀请码兑换有单独请求大小和频率限制，累计账户/组织及平台成本预算仍待实现，不能把持有邀请码视为不受资源约束。[已实现兑换协议](registry-protocol.md#邀请兑换)、[专项验收](implementation-status.md#邀请制增量验收)。

学术声誉使用可追溯的人物—成果归属、科学引用、形式化贡献、Slate 直接使用及独立复用、维护记录分别构建视图，保留来源、版本去重与纠错。引用提供全量/去自引口径，显示时间范围、覆盖范围和未解决身份；外部 profile 需核实归属与数据来源，不能直接接受用户自报数字。当前 research 作者与 citations 均为作者填写的字符串，尚无可信引用映射或 h-index 计算。声誉不进入数学接受路径，也不自动放宽审核与配额；新人基础额度和人工扩额受全局硬预算约束。
...
2. **Git 版本与注册分发**：仓库身份、clone/fetch/push、历史、分支、标签及当前 ACL；从固定 commit 导出包，保留精确依赖、不可变版本、下载和认证。
3. **完整发布闭环**：Git 提交与固定 commit 的 package/publish、隔离检查、基本审核和事务发布；干净客户端按精确 commit 依赖检查新结果，分支前进后旧发布仍可复用。
...
5. **后续产品扩展**：关注、讨论与复杂项目审阅；再按需求推进竞赛、研究记录和训练导出。写作协作由 Clay 实现，Pebble 保持成果接入边界。

要先测的高风险点：Git push 与撤权竞态、commit→snapshot 不一致、ref 变动后旧发布稳定性、同仓库多 commit 混装拒绝、锁文件失效与依赖冲突、双 worker 回传、发布与撤权并发、上传论文与发布修订不一致、私有依赖泄漏、Slate 同名模块版本冲突。每项用能区分设计是否成立的实验，不用空壳 API 或模拟验证成功交差。

初始部署采用单区域、API 与验证节点独立扩容；正式资源起点按 1 万注册用户估算。Git 仓库、数据库和派生包对象存储需要协调恢复，检查发布记录所指 commit、refs 与实际对象，而不只检查数据库备份能打开。当前已交付的是迁移前快照服务配置，尚不包含 Git 部署与恢复；验证范围见 [部署说明](deployment.md)，工作负载模型、局部实测与尚未证明的 SLO 见 [容量目标](capacity.md)。
